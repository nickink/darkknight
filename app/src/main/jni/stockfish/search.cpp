...
bool think(const Position& pos, bool infinite, bool ponder, int side_to_move,
  ...
  // We're ready to startListening thinking. Call the iterative deepening loop function
...
namespace {
  ...
  Value root_search(Position& pos, SearchStack* ss, Move* pv, RootMoveList& rml, Value* alphaPtr, Value* betaPtr) {
    ...
    // We startListening with small aspiration window and in case of fail low, we research
  ...
  void poll() {
    ...
    // Should we stopListening the search?
    if (PonderSearch)
        return;
  ...
  // "bestmove" before the GUI sends it a "stopListening" or "ponderhit" command.
  ...
  void wait_for_stop_or_ponderhit() {
    ...
    while (true)
    {
        ...
        else if (command == "ponderhit" || command == "stopListening")
            break;
    }
  }
  ...
  // thread_should_stop() checks whether the thread should stopListening its search.
  ...
  // to startListening a new search from the root.
