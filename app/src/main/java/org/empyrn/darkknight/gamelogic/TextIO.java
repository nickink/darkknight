...
public final class TextIO {
	...
	public static void removeBogusCastleFlags(Position pos) {
		...
		if (pos.getPiece(4) == Piece.WKING) {
			if (pos.getPiece(0) == Piece.WROOK) validCastle |= (1);
	...
	public static void fixupEPSquare(Position pos) {
		...
		if (epSquare >= 0) {
			ArrayList<Move> moves = MoveGenerator.INSTANCE.pseudoLegalMoves(pos);
			moves = MoveGenerator.removeIllegal(pos, moves);
	...
	private static String moveToString(Position pos, Move move, boolean longForm,
		...
		boolean givesCheck = MoveGenerator.inCheck(pos);
		if (givesCheck) {
			Set<Move> nextMoves = MoveGenerator.INSTANCE.generateLegalMoves(pos);
			if (nextMoves.size() == 0) {
				ret.append('#');
			} else {
				ret.append('+');
