...
public class ChessBoardView extends View {
	...
	private ChessBoardEventListener mBoardEventListener;
	...
	public ChessBoardEventListener getEventListener() {
		return mBoardEventListener;
	}

	public void setEventListener(ChessBoardEventListener mBoardEventListener) {
		this.mBoardEventListener = mBoardEventListener;
		setOnTouchListener(mBoardEventListener);
	}
	...
	public final void setSelectionFromMove(Move m) {
	...
	Move mousePressed(int sq) {
		...
		if (selectedSquare != -1) {
			...
			if (!myColor(p)) {
				clearSelection(); // Remove selection of opponents last moving
				// piece
			}
		}
		...
		int p = pos.getPiece(sq);
		if (selectedSquare != -1) {
			if (sq != selectedSquare && !myColor(p)) {
				return new Move(selectedSquare, sq, Piece.EMPTY);
			} else {
				clearSelection();
			}
		} else {
	...
	public final void clearMoveHints() {
		setMoveHints(null);
	}
